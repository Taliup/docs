...
Taliup POS acts as a sales recording system (SRS / MEV) for Revenu Québec's WEB-SRM. A Québec merchant cannot bill on a terminal until that device is enrolled and the signed-in user is registered with the WEB-SRM.

Revenu Québec requires both steps. Repeat them on every terminal.
...
- The terminal is set up and staff can log in. See [Taliup POS overview](/taliup-pos/index) and [PAX A920 Pro first setup](/taliup-pos/devices/pax/a920-pro).
...
- The terminal is online. The certificate request is sent to Revenu Québec.
...
After you log in, **Set up this terminal** blocks **Register**, **Tables**, and **Menu** until both steps are complete.
...
<Steps>
  ...
  <Step title="Open the certificate request">
    On **Set up this terminal**, tap **Request a certificate**.
  ...
  <Step title="Name the certificate">
    Enter a **Certificate nickname (SN)** of 8 to 32 characters. Use a name you can recognize later on this terminal.
  ...
  <Step title="Register your user">
    Return to **Set up this terminal**. Step 2 is now available. Tap **Register** to record the signed-in employee with the WEB-SRM.

    The terminal can bill only after this step succeeds.
  </Step>
</Steps>
...
| **Device identifier (IDAPPRL)** | Identifier Revenu Québec assigned to this terminal |
...
<Tip>
  Keep a record of the **Device identifier**. You need it if you restore this terminal and choose **Yes** on the certificate form.
</Tip>

## Renew the certificate

Renew when Taliup POS warns that the certificate is expiring, or when **Set up this terminal** asks you to renew because the certificate is expired or not yet valid.
...
Delete only when you are decommissioning the terminal or starting enrolment over.
...
After deletion, **Set up this terminal** returns. Request a new certificate, then register the signed-in user.
...
| Certificate request failed | Confirm the terminal is online and HQ credentials are correct, then tap **Request a certificate** again. The nickname must be 8 to 32 characters. |
...
| Certificate renewal did not complete | Confirm the terminal is online, then tap **Renew certificate** again. |
