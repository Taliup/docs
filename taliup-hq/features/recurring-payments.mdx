...
| **Needs attention** | Past due, grace period, paused, expired checkout, or a card that is about to expire |
| **Active** | Agreements that are billing |
| **Pending** | Waiting for the customer to complete checkout |
| **Closed** | Completed, cancelled, or failed setup |

Search by client name, email, or amount (`100` or `$100`), then **Apply**. Newest agreements appear first. **Reset** clears the search and keeps the tab you are on.

Click a row to open the agreement. **Copy link** copies the checkout link (pending) or the customer manage link (enrolled).
...
<Frame caption="The New recurring agreement form with client, plan items, schedule, and per-cycle total.">
  ![New recurring agreement for Jane Doe with a Subscription Fee item, annually frequency, a 7-day free trial, and CAD $113.00 per-cycle total](/images/taliup-hq/recurring-payments/create.png)
</Frame>
...
| **Location** | The location whose Converge gateway collects the charges. Hidden when the entity has only one location. |
...
You cannot change catalog items or the schedule after the agreement is created. Create a new agreement if the plan needs to change.

### Schedule

| Setting | Description |
|---|---|
| **Frequency** | **Daily**, **Weekly**, **Biweekly**, **Monthly**, or **Annually**. Each cycle is charged at the start of the period. The Settings default prefills this. |
| **Offer a free trial** | The customer authorizes a card now. Charging starts after the trial. Hides **First charge**. |
| **Trial length** | 1–90 days. The form shows the first-charge date. |
| **First charge** | Shown when the trial is off. Today charges the card at checkout. A later date saves the card and charges then. |
...
<Frame caption="The Agreement page with status, first charge, card, customer manage link, items, and charges.">
  ![Active agreement for Jane Doe showing CAD $20.00 monthly, first charge and next payment dates, VISA ending in 0002, customer manage link, and a captured charge](/images/taliup-hq/recurring-payments/show.png)
</Frame>
...
Customers can cancel from the manage page only when **Allow this customer to cancel** is on for that agreement. They confirm in a sheet, then the page shows who cancelled.

You cannot change the amount, frequency, or dates after enrollment. That needs a new customer authorization. Create a new agreement if the plan must change.
...
- **Checkout link** — one-time page to pay today, or to save a card when the first charge is later or a trial is on. The page follows the entity language. Customers can switch to English, French, or Mexican Spanish. They must check **I authorize {merchant} to charge my card…**
...
| Email | When it sends |
|---|---|
| Checkout invitation | When you create an agreement, and when you click **Resend email**. It says whether they pay today or only authorize a card. |
