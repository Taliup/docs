...
<Warning>
  **Update existing** changes the merchant's live data. It only touches fields the draft actually carries, and it adds attributes and taxes rather than replacing them — but the name, description and price will be overwritten. Use **Link existing** when you only want to reuse a record.
</Warning>
...
Background work runs on a dedicated `catalog-builder` queue, so the queue worker must include it:
