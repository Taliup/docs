...
- Si le premier prélèvement est aujourd'hui et qu'il n'y a pas d'essai, le paiement encaisse ce cycle et démarre le calendrier.
- Si le premier prélèvement est plus tard, ou si vous offrez un essai gratuit, le paiement enregistre seulement la carte. Les prélèvements commencent à la date du premier prélèvement.
...
| **Needs attention** | En souffrance, période de grâce, en pause, lien expiré, ou carte bientôt expirée |
| **Active** | Accords en cours de facturation |
| **Pending** | En attente que le client termine le paiement |
| **Closed** | Terminés, annulés, ou échec de configuration |

Recherchez par nom de client, courriel ou montant (`100` ou `$100`), puis **Apply**. Les accords les plus récents apparaissent en premier. **Reset** efface la recherche et garde l'onglet en cours.

Cliquez une ligne pour ouvrir l'accord. **Copy link** copie le lien de paiement (en attente) ou le lien de gestion client (après inscription).
...
<Frame caption="Le formulaire New recurring agreement avec client, articles, calendrier et total par cycle.">
  ![New recurring agreement pour Jane Doe avec un article Subscription Fee, fréquence Annually, un essai gratuit de 7 jours et un total de 113,00 $ CA par cycle](/images/taliup-hq/recurring-payments/create.png)
</Frame>
...
<Steps>
  ...
  <Step title="Définir le calendrier">
    Choisissez la fréquence, puis définissez le premier prélèvement ou offrez un essai gratuit. Ajoutez au besoin une date de fin.
  </Step>
...
| **Location** | L'emplacement dont la passerelle Converge encaisse les prélèvements. Masqué si l'entité n'a qu'un emplacement. |
...
Vous ne pouvez pas changer les articles du catalogue ni le calendrier après la création. Créez un nouvel accord si le forfait doit changer.

### Calendrier

| Réglage | Description |
|---|---|
| **Frequency** | **Daily**, **Weekly**, **Biweekly**, **Monthly** ou **Annually**. Chaque cycle est prélevé au début de la période. La valeur par défaut des Settings préremplit ce champ. |
| **Offer a free trial** | Le client autorise une carte maintenant. Les prélèvements commencent après l'essai. Masque **First charge**. |
| **Trial length** | 1 à 90 jours. Le formulaire affiche la date du premier prélèvement. |
| **First charge** | Affiché lorsque l'essai est désactivé. Aujourd'hui encaisse la carte au paiement. Une date plus tard enregistre la carte et prélève ensuite. |
...
La barre latérale affiche le sous-total, les taxes et le total de chaque cycle. C'est le montant que Converge prélève après l'inscription (et au paiement si le premier prélèvement est aujourd'hui et qu'il n'y a pas d'essai).
...
<Frame caption="La page Agreement avec statut, premier prélèvement, carte, lien de gestion client, articles et prélèvements.">
  ![Accord Active pour Jane Doe montrant 20,00 $ CA par mois, dates de premier prélèvement et de prochain paiement, VISA se terminant par 0002, lien de gestion client et un prélèvement capturé](/images/taliup-hq/recurring-payments/show.png)
</Frame>
...
- Un prélèvement apparaît sous **Charges** lorsque le premier prélèvement est aujourd'hui. Si le premier prélèvement est plus tard, ou s'il s'agit d'un essai gratuit, la carte est enregistrée et rien n'est prélevé.
- Le statut passe à **Active**.
- Le **Customer manage link** apparaît. Il n'expire pas. Les clients l'utilisent pour voir l'historique, mettre à jour leur carte et annuler si vous l'avez autorisé.

Si le client annule, ou si vous annulez depuis HQ, l'accord affiche **Cancelled** et qui a annulé : **Customer**, le nom du commerçant, ou **Automatically**.
...
Les clients peuvent annuler depuis la page de gestion seulement si **Allow this customer to cancel** est activé pour cet accord. Ils confirment dans une feuille, puis la page indique qui a annulé.

Vous ne pouvez pas changer le montant, la fréquence ni les dates après l'inscription. Cela exige une nouvelle autorisation du client. Créez un nouvel accord si le forfait doit changer.
...
- **Checkout link** — page à usage unique pour payer aujourd'hui, ou pour enregistrer une carte si le premier prélèvement est plus tard ou si un essai est activé. La page suit la langue de l'entité. Les clients peuvent passer à l'anglais, au français ou à l'espagnol du Mexique. Ils doivent cocher **I authorize {merchant} to charge my card…**
- **Customer manage link** — n'expire pas. Les clients voient le calendrier, l'historique et **Update payment method**. **Cancel recurring payment** n'apparaît que si vous l'avez autorisé. Ils confirment dans une feuille, puis peuvent revenir. La page utilise le même sélecteur de langue.
...
|---|---|
| Invitation de paiement | À la création, et quand vous cliquez sur **Resend email**. Elle indique s'ils paient aujourd'hui ou s'ils autorisent seulement une carte. |
...
Les accords **Failed** se trouvent sous **Closed**. Le client peut avoir besoin d'un nouvel accord et d'un nouveau lien de paiement.

### Le client veut un autre montant ou une autre fréquence

Vous ne pouvez pas changer le calendrier après l'inscription. Annulez l'accord en cours au besoin, puis créez-en un nouveau.

### Le client veut arrêter la facturation

Utilisez **Cancel agreement**, ou demandez-lui d'appuyer sur **Cancel recurring payment** sur la page de gestion si vous l'avez autorisé. Utilisez Pause pour arrêter les prélèvements temporairement.
